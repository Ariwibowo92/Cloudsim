...
/**
 * A Self test simulation will show how to 300 cloudlets distributed equally among
 * 5 Vms with different MIPS requirements.
 */
public class LeastConnectionAnalysisResponseTimeAndCpuLoad {
    ...
    private static List<Vm> createVM(int userId, int vms, int idShift) {
            ...
            for(int i=0;i<vms;i++){
                ...
                int ram = r.ints(1, 100, 1000).findFirst().getAsInt();//512; //vm memory (MB)
...
//                length /=  10;
            //cloudlet parameters
            long length = r.ints(1, 3000, 500000).findFirst().getAsInt();//40000;
...
//		return broker;
    ...
    private static void printCloudletList(List<Cloudlet> list) throws FileNotFoundException {
        ...
        PrintWriter pw = new PrintWriter(new File("LeastConnectionLoadBalancers_RunResult_"+filename));
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            cloudletList = createCloudlet(brokerId, 600, 1); // creating 600 cloudlets

            broker.submitVmList(vmlist);
            //broker.submitCloudletList(cloudletList);
            broker.submitCloudletList(cloudletList);

            //Each cloudlets will need one connection
            //Least connection will give task to a smallest number of connection,
            //it means that the cloudlets (tasks) will be divided equally 
            //among each Vm's by the least connection algorithm,
            ...
            String filename = new SimpleDateFormat("yyyyMMddHHmm'.csv'").format(new Date());
            PrintWriter pw = new PrintWriter(new File("LeastConnectionLoadBalancers_RunResultAverage_"+filename));
            StringBuilder sb = new StringBuilder();
            String line = "";
