...
public class Cloudlet {
    ...
    public double GetCpuLoad(double actualCPUTime) {
        double returnData = 0.0;
        Random r = new Random();
        
        if(actualCPUTime>0 && actualCPUTime<=1500){
            returnData = r.doubles(1, 10, 40).findFirst().getAsDouble();
        }
        
        if(actualCPUTime>1500 && actualCPUTime<=3000){
            returnData = r.doubles(1, 35, 70).findFirst().getAsDouble();
        }
        
        if(actualCPUTime>3000){
            returnData = r.doubles(1, 65, 100).findFirst().getAsDouble();
        }
